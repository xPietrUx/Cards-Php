...
<body>
  <div class="menuH">
    ...
    <div class="CarsChooseH">
      <ul>
        <li><a href="#">Sprzedaż samochodów</a>
  ...
  <div class="ContainerMainH">
        ...
        <form action="CarSalesDownload.php" method="POST" enctype="multipart/form-data">
          <div class="Slajder">
        ...
        <?php
             ...
             $carsales = $mysqli->query("SELECT * FROM carsales ORDER BY ID_CarSales DESC;") or die($mysqli->error);
     
             $carsalesphotos = $mysqli->query("SELECT *
             FROM carsalesphotos 
             WHERE carsalesphotos.ID_CarSalesImage NOT IN (SELECT MIN(carsalesphotos.ID_CarSalesImage) FROM carsalesphotos GROUP BY carsalesphotos.ID_CarSalesImageTo)
             ORDER BY carsalesphotos.ID_CarSalesImageTo;") or die($mysqli->error);

             $carsalesphotosFirst = $mysqli->query("SELECT carsalesphotos.ID_CarSalesImage, carsalesphotos.img_dir, carsalesphotos.ID_CarSalesImageTo FROM carsalesphotos GROUP BY carsalesphotos.ID_CarSalesImageTo;") or die($mysqli->error);

             while($data = $carsales->fetch_assoc())
             {
                 $ID_CarSales = $data['ID_CarSales'];
                 ...
                 {
                      $ID_CarSalesImageMIN = $data1['ID_CarSalesImage'];
                      $img_dir = $data1['img_dir'];
                      $ID_CarSalesImageToMIN = $data1['ID_CarSalesImageTo'];
                ...
                echo '</div>
                 <div class="CarCardBigger">
                   <div id="carouselExampleControlsNoTouching'.$ID_CarSales.'" class="carousel slide" data-bs-touch="false"
                    ...
                    foreach($carsalesphotosFirst as $data1)
                    {
                     ...
                     {
                      $ID_CarSalesImage = $data2['ID_CarSalesImage'];
                      $img_dir = $data2['img_dir'];
                      $ID_CarSalesImageTo = $data2['ID_CarSalesImageTo'];
                 ...
                 <form action="CarSalesDelete.php" method="POST" style="width: 50%">
                     <input type="text" name="WhichToDelete" value="'.$ID_CarSales.'" style="display: none">                     <input type="submit" name="Delete" class="Delete" value="Usuń">
                 </form>
                 </div>
               </div>';
             }
